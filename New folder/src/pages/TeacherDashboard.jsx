...
const TeacherDashboard = () => {
  ...
  const handleAddLesson = async (e) => {
    ...
    try {
      ...
      // const token = localStorage.getItem('token');
      // await axios.post('http://localhost:8000/api/lessons', {
      //   title: lessonTitle, subjectId: lessonSubject, videoUrl: lessonVideoUrl
      ...
      if (!coursesResponse.data || coursesResponse.data.length === 0) {
        alert('لم يتم العثور على كورس لهذه المادة.');
        setSubmitting(false);
        return;
      }
      ...
      // 3. إضافة الدرس للكورس وتحديثه في json-server
      ...
      // 4. إغلاق الـ Modal وإعادة تعيين الحقول
      setShowAddLessonModal(false);
      setLessonTitle('');
      setLessonSubject('');
      setLessonVideoUrl('');

      // 5. إعادة تحميل بيانات الداشبورد
      fetchDashboardData();

      alert('تم إضافة الدرس بنجاح!');
    } catch (err) {
      console.error("Add lesson error:", err);
      alert('فشل إضافة الدرس. تأكد من تشغيل json-server.');
    } finally {
      setSubmitting(false);
    }
  };

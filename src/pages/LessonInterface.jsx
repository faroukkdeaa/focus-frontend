...
const LessonInterface = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        // جلب بيانات الكورس والدروس + تقدم المستخدم
        const storedUser = JSON.parse(localStorage.getItem('user') || '{}');
        const userId = String(storedUser.id || '');

        const [coursesResponse, progressResponse] = await Promise.all([
          axios.get(`http://localhost:3001/courses?subjectId=${subjectId}`),
          userId
            ? axios.get(`http://localhost:3001/user_lesson_progress?userId=${userId}&subjectId=${subjectId}`)
            : Promise.resolve({ data: [] }),
        ]);

        const doneSet = new Set(
          progressResponse.data.filter(p => p.completed).map(p => p.lessonId)
        );
        ...
        if (coursesResponse.data && coursesResponse.data.length > 0) {
          ...
          // دمج حالة الإتمام من بيانات المستخدم
          const merged = (courseData.lessons || []).map(l => ({ ...l, completed: doneSet.has(l.id) }));
          setLessons(merged);
          
          // تحديد الدرس الحالي
          const lesson = merged.find(l => l.id === parseInt(lessonId)) || merged[0];
  ...
  const handleCompleteLesson = async () => {
    ...
    try {
      ...
      // 1. سجّل إتمام الدرس في user_lesson_progress
      await axios.post('http://localhost:3001/user_lesson_progress', {
        userId,
      ...
      // 3. حدّث user_subjects (تقدم المادة)
      const [userSubjectsRes, subjectUnitsRes] = await Promise.all([
        axios.get(`http://localhost:3001/user_subjects?userId=${userId}&subjectId=${subjectId}`),
        axios.get(`http://localhost:3001/subject_units?subjectId=${subjectId}`),
      ]);
      const totalLessonsInSubject = subjectUnitsRes.data.reduce((s, u) => s + u.lessons.length, 0);
      const newCompleted = newDone.size;
      const newProgress = totalLessonsInSubject
        ? Math.round((newCompleted / totalLessonsInSubject) * 100)
        : 0;
      if (userSubjectsRes.data.length > 0) {
        const us = userSubjectsRes.data[0];
        await axios.patch(`http://localhost:3001/user_subjects/${us.id}`, {
          completed: newCompleted,
          progress: newProgress,
        });
      }

      // 4. حدّث user_dashboards stats.completedLessons
      const udRes = await axios.get(`http://localhost:3001/user_dashboards?userId=${userId}`);
      if (udRes.data.length > 0) {
        const ud = udRes.data[0];
        await axios.patch(`http://localhost:3001/user_dashboards/${ud.id}`, {
          stats: { ...ud.stats, completedLessons: (ud.stats?.completedLessons || 0) + 1 },
        });
      }
